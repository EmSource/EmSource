...
#define WRENCH_RANGE_SHORT 32
#define WRENCH_RANGE_LONG 48
...
Vector head_hull_maxs( 16, 16, 18 );
	...
	// Output : void*
	//-----------------------------------------------------------------------------
	void* SendProxy_SendActiveLocalWrenchDataTable( const SendProp *pProp, const void *pStruct, const void *pVarData, CSendProxyRecipients *pRecipients, int objectID )
	{
	...
	REGISTER_SEND_PROXY_NON_MODIFIED_POINTER( SendProxy_SendActiveLocalWrenchDataTable );
	...
	// Output : void*
	//-----------------------------------------------------------------------------
	void* SendProxy_SendActiveLocalWrenchGGDataTable( const SendProp *pProp, const void *pStruct, const void *pVarData, CSendProxyRecipients *pRecipients, int objectID )
	{
		// Get the weapon entity
		CBaseCombatWeapon *pWeapon = (CBaseCombatWeapon*)pVarData;
		if ( pWeapon )
	...
	REGISTER_SEND_PROXY_NON_MODIFIED_POINTER( SendProxy_SendActiveLocalWrenchGGDataTable );
#endif
...
IMPLEMENT_NETWORKCLASS_ALIASED( Wrench, DT_WeaponWrench )

BEGIN_NETWORK_TABLE( CWrench, DT_WeaponWrench )
END_NETWORK_TABLE()

BEGIN_PREDICTION_DATA( CWrench )
END_PREDICTION_DATA()


LINK_ENTITY_TO_CLASS_ALIASED( weapon_wrench, Wrench );
PRECACHE_REGISTER( weapon_wrench );


// ----------------------------------------------------------------------------- //
// CWrench implementation.
// ----------------------------------------------------------------------------- //
...
bool CWrench::HasPrimaryAmmo()
{
	return true;
}


bool CWrench::CanBeSelected()
{
	return true;
}
...
void CWrench::Precache()
{
	...
	PrecacheScriptSound( "Weapon_Wrench.Deploy" );
	PrecacheScriptSound( "Weapon_Wrench.Slash" );
	PrecacheScriptSound( "Weapon_Wrench.Stab" );
	PrecacheScriptSound( "Weapon_Wrench.Hit" );
	PrecacheEffect( "WrenchSlash" );
}
...
void CWrench::WeaponAnimation ( int iAnimation )
{
	/*
	int flag;
	...
	PLAYBACK_EVENT_FULL( flag, pPlayer->edict(), m_usWrench,
...
void CWrench::PrimaryAttack()
{
	...
	if ( pPlayer )
	{
		...
		lagcompensation->StartLagCompensation( pPlayer, LAG_COMPENSATE_HITBOXES_ALONG_RAY, pPlayer->EyePosition(), pPlayer->EyeAngles(), WRENCH_RANGE_LONG );
#endif
...
#if !defined (CLIENT_DLL)
		// Move other players back to history positions based on local player's lag
		lagcompensation->StartLagCompensation( pPlayer, LAG_COMPENSATE_HITBOXES_ALONG_RAY, pPlayer->EyePosition(), pPlayer->EyeAngles(), WRENCH_RANGE_SHORT );
#endif
...
bool CWrench::SwingOrStab( CSWeaponMode weaponMode )
{
	CCSPlayer *pPlayer = GetPlayerOwner();
	if ( !pPlayer )
		return false;
...
#ifndef CLIENT_DLL
	pPlayer->PlayerUsedWrench();
#endif

	// bStab: false=primary, true=secondary
	float fRange = (weaponMode == Primary_Mode) ? WRENCH_RANGE_LONG : WRENCH_RANGE_SHORT; // knife range
	...
	if ( bDidHit )
	{
		...
		flDamage *= ( WrenchDamageScale.GetInt() / 100.0f );
#endif
		...
		if ( tr.m_pEnt )
		{
			...
			{
				EmitSound( filter, entindex(),  (weaponMode == Secondary_Mode) ? "Weapon_Wrench.Stab" : "Weapon_Wrench.Hit" );
			}
			else
			{
				EmitSound( filter, entindex(), "Weapon_Wrench.HitWall" );
			}
		}
		...
		DispatchEffect( filter, 0.0, "WrenchSlash", data );
	}
	else
	{
		...
		CPASAttenuationFilter filter( this );
		filter.UsePredictionRules();
		EmitSound( filter, entindex(), "Weapon_Wrench.Slash" );
...
IMPLEMENT_NETWORKCLASS_ALIASED( WrenchGG, DT_WeaponWrenchGG )

BEGIN_NETWORK_TABLE( CWrenchGG, DT_WeaponWrenchGG )
END_NETWORK_TABLE()

BEGIN_PREDICTION_DATA( CWrenchGG )
END_PREDICTION_DATA()

LINK_ENTITY_TO_CLASS_ALIASED( weapon_knifegg, WrenchGG );
